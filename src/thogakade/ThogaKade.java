package thogakade;

import customer.ViewCustomerForm;
import customer.UpdateCustomerForm;
import customer.AddCustomerForm;
import item.AddItemForm;
import item.DeleteItemForm;
import item.SearchItemForm;
import item.UpdateItemForm;
import item.ViewItemForm;

public class ThogaKade {

    public static void main(String[] args) {
//        AddCustomerForm addCustomerForm = new AddCustomerForm();
//        SearchCustomerForm searchCustomerForm = new SearchCustomerForm();
//        UpdateCustomerForm updateCustomerForm = new UpdateCustomerForm();
//        DeleteCustomerForm deleteCustomerForm = new DeleteCustomerForm();
//        ViewCustomerForm viewCustomerForm = new ViewCustomerForm();
        
       AddItemForm addItemForm = new AddItemForm();
       SearchItemForm searchItemForm = new SearchItemForm();
       DeleteItemForm deleteItemForm = new DeleteItemForm();
       UpdateItemForm updateItemForm = new UpdateItemForm();
       ViewItemForm viewItemForm = new ViewItemForm();
    }
    
}
